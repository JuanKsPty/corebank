 ...
 *
 * The assistant sits alongside rather than below, because the two are meant to be
 * used together: you ask it something and watch the balance answer.
 */
export function DashboardPage() {
  ...
  if (dashboard.isError) {
    return (
      <Notice tone="error" title="No pudimos cargar tu resumen">
        Vuelve a intentarlo en un momento. Si sigue pasando, revisa que la API esté
        en marcha.
      </Notice>
    )
  }
  ...
  const ownedAccounts = new Set((data?.accounts ?? []).map((account) => account.account_number))
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_23rem]">
      <div className="min-w-0 space-y-6">
        ...
        <section aria-labelledby="saldo">
          ...
          <p className="mt-1 text-[0.9375rem] text-ink-soft">
            {firstName ? `${firstName}, esto es lo que puedes mover ahora.` : 'Esto es lo que puedes mover ahora.'}
          </p>
        ...
        <section aria-labelledby="cuentas">
          <div className="mb-3 flex items-baseline justify-between gap-3">
            ...
            <Link to="/mover" className="text-[0.8125rem] font-medium text-copper hover:underline">
              Mover dinero
            </Link>
          </div>
          ...
          {dashboard.isLoading ? (
            ...
            <div className="card">
              <EmptyState title="No tienes cuentas abiertas">
                Tu cuenta se crea al registrarte. Si ves esto, algo salió mal en el
                registro.
              </EmptyState>
            </div>
          )}
      ...
      {/* On a narrow screen the assistant moves under the accounts rather than
          into a tab: it is the feature worth discovering, and a tab hides it. */}
      <div className="min-w-0 lg:sticky lg:top-6 lg:self-start">
        <ChatPanel pendingConfirmations={data?.pending_confirmations} />
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between gap-2">
        <p className="text-[0.8125rem] font-medium">{accountTypeLabel(account.account_type)}</p>
        <p className="type-figure text-[0.6875rem] text-ink-faint">
      ...
      {held ? (
        ...
        <p className="type-figure mt-1 text-[0.75rem] text-ink-faint">{account.account_number}</p>
      )}
    </Link>
  )
}
