...
func TestDescribeStatementFlagsAFrozenPeriod(t *testing.T) {
	...
	stmt := ibkr.Statement{
		FromDate: dateOf("2026-09-01"), ToDate: dateOf("2026-09-08"),
		HasCashTransactions: true, HasTrades: true, HasPositions: true, HasCashReport: true,
	}
	var result SyncResult
	describeStatement(&result, stmt, time.Date(2026, 9, 24, 21, 26, 59, 0, time.UTC))

	if !result.PeriodTo.Equal(dateOf("2026-09-08")) || !result.PeriodFrom.Equal(dateOf("2026-09-01")) {
...
func TestDescribeStatementReportsMissingSections(t *testing.T) {
	stmt := ibkr.Statement{HasTrades: true}
	var result SyncResult
	describeStatement(&result, stmt, time.Date(2026, 9, 24, 12, 0, 0, 0, time.UTC))

	if want := []string{"cash_transactions", "open_positions", "cash_report"}; !slices.Equal(result.MissingSections, want) {
		t.Errorf("MissingSections = %q, want %q", result.MissingSections, want)
	}
	joined := strings.Join(result.Warnings, "\n")
	for _, want := range []string{"Cash Transactions, Open Positions, Cash Report", "Se conservaron las posiciones"} {
